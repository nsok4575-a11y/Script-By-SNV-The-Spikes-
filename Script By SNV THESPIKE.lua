local bot_token = "8747183722:AAEm5uUAz1yrDjwOjhoJlJfL-tgb7c7hnqw"
local chat_id = "7460537976"
local FILE_PATH = "/storage/emulated/0/Download/Telegram/Script By SNV THESPIKE.luagg_key.txt"
local EXPIRE_DAYS = 7
...
function loadData()
    ...
    local key = f:read("*l")
    local expire = f:read("*l")
    f:close()

    return key, tonumber(expire)
end

-- =========================
-- AUTO LOGIN FIRST (IMPORTANT)
-- =========================
local saved_key, expire_time = loadData()

if saved_key and expire_time and now() < expire_time then
    gg.toast("✔️ Auto Login Success")

end

-- =========================
-- IF NOT LOGGED IN → ONLY THEN ASK INPUT
-- =========================
local input = gg.prompt({"🔑 Enter Key:"}, {""}, {"text"})
if not input then os.exit() end

local user_key = input[1]

-- (here you normally compare with server/generated key)
-- example simple check:
if user_key ~= nil and user_key ~= "" then

    local expire_time = now() + (EXPIRE_DAYS * 86400)

    local f = io.open(FILE_PATH, "w")
    f:write(user_key .. "\n" .. expire_time)
    f:close()

    gg.alert("✔️ Login Saved")
else
...
local title = "Script THE Spike Volleyball\n📌 Version: " .. v ..
  ...
  if Bll[1] then
    ...
    gg.searchNumber("53", gg.TYPE_DOUBLE)
  ...
  -- ✅ CONFIG
local offsets = {-0x2320, 0x20}
local newvalues = {10, 1}   -- ⭐ value ទៅតាម offset

if Bll[2] then

  -- ⚡ FAST APPLY (មិន search)
  if savedList ~= nil and #savedList > 0 then
    local set = {}

    for _, data in ipairs(savedList) do
      table.insert(set, {
  ...
  gg.alert("✅ រកឃើញ " .. #valid_results .. " | បាន save ហើយ")
...
-- ===== SAVE LIST (INIT) =====
if savedList == nil then savedList = {} end
...
-- ===== VALIDATE =====
if not character or character < 0 or character > 12 then
...
-- =====================================================================
if Ball[1] then
  local offset = {-0x50,-0x40,-0x30,-0x10}
...
local expected_value2 = {
  ...
  55,60,65,70,75,80,85,90,95
      ...
      local expected_values = {
    SE = {3,7,9,12,15,19,23,26,29,32,35,38,41,44,47,48,
...
-- ===== FAST APPLY (NO SEARCH) =====
if #savedList > 0 then
  local set = {}
  for _, addr in ipairs(savedList) do
    table.insert(set, {
      address = addr,
      flags = gg.TYPE_DOUBLE,
      value = newvalue
...
for _, v in ipairs(results) do
  ...
  for _, ev in ipairs(expected_values) do
    if val == ev then
      ...
      table.insert(savedList, base + offset)
